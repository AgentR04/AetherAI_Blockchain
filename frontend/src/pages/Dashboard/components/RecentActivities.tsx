...
interface Activity {
  id: string;
  type: 'risk_assessment' | 'anomaly' | 'liquidity' | 'authentication';
  title: string;
  details: string;
  timestamp: string;
  riskScore?: number;
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Chip
                    ...
                    size="small"
                    sx={{ bgcolor: getChipColor(activity.type), color: 'white' }}
                  />
                  {activity.riskScore !== undefined && (
                    <Chip
                      label={`Risk Score: ${(activity.riskScore * 100).toFixed(0)}%`}
                      size="small"
                      sx={{
                        bgcolor: activity.riskScore < 0.3 ? '#4CAF50' : activity.riskScore < 0.6 ? '#FFA726' : '#FF5252',
                        color: 'white'
                      }}
                    />
                  )}
                  {activity.biometricConfidence !== undefined && (
                    <Chip
                      label={`Biometric Confidence: ${(activity.biometricConfidence * 100).toFixed(0)}%`}
                      size="small"
                      sx={{
                        bgcolor: activity.biometricConfidence > 0.8 ? '#4CAF50' : '#FFA726',
                        color: 'white'
                      }}
                    />
                  )}
